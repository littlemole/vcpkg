...
#################################################################
# hack to support multiple archs for both static and dll build

# make sure directories exist

!if [if not exist "$(DIR_BIN)\$(null)" mkdir "$(DIR_BIN)"]
!endif

!if [if not exist "$(DIR_O)\$(null)" mkdir "$(DIR_O)"]
!endif

# handle architecture. sets the MACHINE macro used later

!IF "$(ARCH)"=="x86"
MACHINE=
!ELSE IF "$(ARCH)"=="x64"
MACHINE=/Machine:X64 /ignore:4044 /ignore:4006
!ENDIF

# handle static or dll build. sets the DLL and SCI_STATIC macro used later

!IF "$(TARGET)"=="dll"
DLL=-DEF:Scintilla.def -DLL
LD=link
...
SCI_STATIC=-DSTATIC_BUILD
!ENDIF

# end of hack. but note the use of 
# SCI_STATIC, DLL and MACHINE 
# macros below
###############################################################
...
CRTFLAGS=-D_CRT_SECURE_CPP_OVERLOAD_STANDARD_NAMES=1 -D_CRT_SECURE_NO_DEPRECATE=1 -D_SCL_SECURE_NO_WARNINGS=1 $(XP_DEFINE) $(SCI_STATIC)
CXXFLAGS=-Zi -TP -MP -W4 -EHsc -std:c++17 $(CRTFLAGS)
CXXDEBUG=-Od -MTd -DDEBUG
CXXNDEBUG=-O1 -MT -DNDEBUG -GL
NAME=-Fo
LDFLAGS=-OPT:REF -LTCG -IGNORE:4197 -DEBUG $(XP_LINK) $(MACHINE)
LDDEBUG=
